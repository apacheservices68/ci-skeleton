	...
	protected function delete_file($state_info)
	{
		...
		if(isset($state_info->field_name) && isset($this->upload_fields[$state_info->field_name]))
		{
			...
			if(file_exists("{$upload_info->upload_path}/{$state_info->file_name}"))
			{
				...
				{
				    # Update 16/06/2014 7.00
				    unlink("{$upload_info->upload_path}/thumb_{$state_info->file_name}");
                    # Update 16/06/2014 7.00
                    # Update 01/07/2014 7.00
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function get_integer_input($field_info,$value)
	{
		...
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' class='form-control ' value='$value' class='numeric' $extra_attributes />";
		return $input;
	}
	...
	protected function get_string_input($field_info,$value)
	{
		...
		if(!empty($field_info->db_max_length))
			$extra_attributes .= "maxlength='{$field_info->db_max_length}'";
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' class='form-control ' value=\"$value\" $extra_attributes />";
		return $input;
	}
	...
	protected function get_text_input($field_info,$value)
	{
		if($field_info->extras == 'text_editor')
		{
			...
			switch ($editor) {
				case 'ckeditor':
					$this->set_js_lib($this->default_texteditor_path.'assets/grocery_crud/texteditor/ckeditor/ckeditor.js');
					$this->set_js_lib($this->default_texteditor_path.'assets/grocery_crud/texteditor/ckeditor/adapters/jquery.js');
					$this->set_js_config($this->default_javascript_path.'/jquery_plugins/config/jquery.ckeditor.config.js');
				break;

				case 'tinymce':
					$this->set_js_lib($this->default_texteditor_path.'assets/grocery_crud/texteditor/tiny_mce/jquery.tinymce.js');
					$this->set_js_config($this->default_javascript_path.'/jquery_plugins/config/jquery.tine_mce.config.js');
				break;

				case 'markitup':
					$this->set_css($this->default_texteditor_path.'assets/grocery_crud/texteditor/markitup/skins/markitup/style.css');
					$this->set_css($this->default_texteditor_path.'assets/grocery_crud/texteditor/markitup/sets/default/style.css');

					$this->set_js_lib($this->default_texteditor_path.'assets/grocery_crud/texteditor/markitup/jquery.markitup.js');
	...
	protected function get_datetime_input($field_info,$value)
	{
		...
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' class='form-control ' value='$datetime' maxlength='19' class='datetime-input' />
	...
	protected function get_date_input($field_info,$value)
	{
		...
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' class='form-control ' value='$date' maxlength='10' class='datepicker-input' />
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	const	JQUERY 			= "jquery-1.11.0.min.js";
    ...
    public function delete() {
        ...
        if ($success) {
            foreach($this->options['image_versions'] as $version => $options) {
                ...
                # Options delete Thumbnails
                # Update 16/06/2014 7.00  
                $file1 = $options['upload_dir'].'thumb_'.$file_name;
                # Update 01/07/2014 7.00  
                //$file2 = $options['upload_dir'].'thumb-'.$file_name;
                # End Options delete Thumbnails 
                if (is_file($file)) {
                    unlink($file);
                    # Options delete Thumbnails 
                    unlink($file1);
